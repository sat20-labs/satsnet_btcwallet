...
var (
	...
	// walletDataDir  = btcutil.AppDataDir("btcwallet", false)
	// walletConfigFile  = filepath.Join(walletDataDir, defaultConfigFilename)
	// walletRPCKeyFile  = filepath.Join(walletDataDir, "rpc.key")
	// walletRPCCertFile = filepath.Join(walletDataDir, "rpc.cert")
	// walletLogDir      = filepath.Join(walletDataDir, defaultLogDirname)
	btcdDataDir       string
	btcdCAFile        string
	walletDataDir     string
	walletConfigFile  string
	walletLogDir      string
	walletRPCKeyFile  string
...
func loadConfig() (*config, []string, error) {
	...
	preCfg := config{
		ConfigFile: cfgutil.NewExplicitString(""),
		AppDataDir: cfgutil.NewExplicitString(""),
		CAFile:     cfgutil.NewExplicitString(""),
		RPCKey:     cfgutil.NewExplicitString(""),
		RPCCert:    cfgutil.NewExplicitString(""),
	...
	btcdDataDir = filepath.Join(homeDir, "btcd")
	btcdCAFile = filepath.Join(btcdDataDir, "rpc.cert")

	walletDataDir := filepath.Join(homeDir, "btcwallet")
	walletConfigFile = filepath.Join(walletDataDir, defaultConfigFilename)
	walletRPCKeyFile = filepath.Join(walletDataDir, "rpc.key")
	walletRPCCertFile = filepath.Join(walletDataDir, "rpc.cert")
	walletLogDir = filepath.Join(walletDataDir, defaultLogDirname)
	...
	cfg := config{
		...
		ConfigFile:             cfgutil.NewExplicitString(walletConfigFile),
		AppDataDir:             cfgutil.NewExplicitString(walletDataDir),
		LogDir:                 walletLogDir,
		WalletPass:             wallet.InsecurePubPassphrase,
		CAFile:                 cfgutil.NewExplicitString(""),
		RPCKey:                 cfgutil.NewExplicitString(walletRPCKeyFile),
		RPCCert:                cfgutil.NewExplicitString(walletRPCCertFile),
		LegacyRPCMaxClients:    defaultRPCMaxClients,
		LegacyRPCMaxWebsockets: defaultRPCMaxWebsockets,
		DataDir:                cfgutil.NewExplicitString(walletDataDir),
		UseSPV:                 false,
		AddPeers:               []string{},
		ConnectPeers:           []string{},
	...
	if preCfg.ConfigFile.ExplicitlySet() {
		// Not set config file in command line
		configFilePath = cfg.ConfigFile.Value // use default config file with homedir
		configFilePath = cleanAndExpandPath(configFilePath)
	} else {
		// Set config file in command line
		appDataDir := walletDataDir // use default
		if preCfg.AppDataDir.ExplicitlySet() {
			// set Wallet data directory in command line
			appDataDir = cleanAndExpandPath(preCfg.AppDataDir.Value)
		} else if preCfg.DataDir.ExplicitlySet() {
			// set Wallet data directory in command line with datadir
	...
	if cfg.SatsNet {
		numNets++
		activeNet = &netparams.SatsMainNetParams
	}
	if cfg.SatsTestNet {
		numNets++
		activeNet = &netparams.SatsTestNetParams
	}
